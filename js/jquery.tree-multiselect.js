...
Tree.prototype.createSelectAllButtons = function (parentNode) {
  ...
  this.$selectionContainer.on('click', 'span.select-all', function () {
    self.keysToAdd = Object.keys(self.astItems).filter(function(key) {return self.astItems[key].disabled==false;})
    self.render();
  });
...
Tree.prototype.render = function (noCallbacks) {
  ...
  for (var kk = 0; kk < this.selectedKeys.length; ++kk) {
    ...
    originalValsHash[this.selectedKeys[kk]] = true;//!!!this.astItems[this.selectedKeys[kk]].disabled;
    valHash[value] = kk;
  }
...
exports.createSelection = function (astItem, createCheckboxes, disableCheckboxes) {
  ...
  if (hasDescription) {
    var popup = exports.createNode('span', { class: 'description', text: astItem.description });
